#!/usr/bin/env python3
"""Transparent PNG cutouts + SVG wrappers for boy 1 poses."""
...
DST = Path("/Users/crawfordjpaul/Local Sites/rr-website/src/lib/assets/characters/boy 1")
POSES = ["idle", "wave", "pencil", "point", "guitar"]
CREAM = np.array([255.0, 242.0, 222.0])
...
def remove_background(im: Image.Image, pose: str) -> Image.Image:
    ...
    # Eyes / laces / toes — exclude cream bg (cream is warmer ~29)
    is_cool_white = (
        (mx > 215)
        & (chroma < 45)
        & (warm < 22)
    ...
    hard_protect = (
        is_blue | is_dark | is_wood | is_skin | is_cool_white | is_notebook | is_pencil_yellow
    )
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 {ww} {hh}" width="{ww}" height="{hh}">
  <title>Rhythm Roots boy 1 — {name}</title>
